...
I have also just realised why the goem_points are being highlighted.
This is the same as with the Ci for the goem_smooth. IT is becuase only stroke is affected and geom_points are all fill? Ok I forogt about the use tag. i gave it a little bit of a try but it still didnt work. Maybe instead you could have the use tag as the one with the ID rather than  separate parent g tag.

#### NA ins multi layes
As descovred right at the end of the geom_bar seciton there is a problem where the goem_lin doesnt work where there are NA values and multiple layers.

I have made the svg so that it splits each lines sections into more sections. This means that you will be able ot click through the disjoint line. I need to add the xml to back this up.
...
```{r}
singleLine = ggplot(starwars, aes(mass, height)) +
  geom_line()
